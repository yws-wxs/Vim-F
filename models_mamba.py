...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride=16, in_chans=3, embed_dim=192, norm_layer=None, flatten=True,
        ...
        self.sup_c =  nn.Parameter(torch.ones(2))  #时频域叠加系数

        self.cnn_downsample_layers = nn.ModuleList()

        stem = nn.Sequential(  # 下采样4倍（stride=16）或2倍（stride=8）
    ...
    def forward(self, x):
        ...
        x_f = torch.abs(torch.fft.fftshift(torch.fft.fft2(x, norm='ortho')))  #计算频谱图
        x = x*self.sup_c[0] + x_f*self.sup_c[1] #叠加
        x = self.norm(x)
        return x
