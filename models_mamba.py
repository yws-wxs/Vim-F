...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride=16, in_chans=3, embed_dim=192, norm_layer=None, flatten=True,
        ...
        stem = nn.Sequential(                  #3*224*224------->48*56*56
        ...
        self.FFT_w = nn.Parameter(torch.ones((img_size // patch_size) ** 2, embed_dim))
...
def vim_tiny_patch16_224_bimambav2_final_pool_mean_abs_pos_embed_with_midclstok_div2(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, embed_dim=192, depth=24, rms_norm=True, residual_in_fp32=True, fused_add_norm=True, final_pool_type='mean', if_abs_pos_embed=False, if_rope=False, if_rope_residual=False, bimamba_type="v2", if_cls_token=True, if_devide_out=True, use_middle_cls_token=True, **kwargs)
